...
If `pubkey` is set, the signer must sign with that key.
...
async window.nostr.nip44.decrypt(pubkey, ciphertext, self?): string // takes ciphertext as specified in nip-44
```

If `self` is set, the signer must use that key of the user (`pubkey` being the other party's).
